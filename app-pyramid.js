...
class AppPyramidClass {
  ...
  loadWins() {
    try {
      const savedWins = localStorage.getItem('zoo_pyramid_wins');
      if (savedWins) {
    ...
    if (!this.wins || typeof this.wins !== 'object') {
      this.wins = { 3: 0, 4: 0, 5: 0, 6: 0 };
    }
    if (this.wins[3] === undefined) this.wins[3] = 0;
    if (this.wins[4] === undefined) this.wins[4] = 0;
    if (this.wins[5] === undefined) this.wins[5] = 0;
    if (this.wins[6] === undefined) this.wins[6] = 0;
  }

  saveWins() {
    try {
      localStorage.setItem('zoo_pyramid_wins', JSON.stringify(this.wins));
    } catch(e) {}
  }
